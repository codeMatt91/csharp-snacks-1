...
// Dato unb vettore di interi, costruito con numeri casuali, calcolare la media 
...
while (list.ToArray().Length < 10000)
{
    int n2 = rnd.Next(0,1000);
    tot += n2;
    list.Add(n2);

}

media = tot / list.ToArray().Length;
Console.WriteLine("La media e: {0}",media);
foreach (int j in list.ToArray())
    Console.Write("{0} ", j);
Console.WriteLine();
